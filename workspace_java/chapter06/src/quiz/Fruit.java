...
//	이름 배열
//	가격 배열
	String[] fruitName = {"사과", "청포도", "망고", "용과"};
	int[] fruitPrice = {6000, 5000, 9000, 8000};
	

/* 생성자 */
//	가게명 필수
	Fruit(String marketName) {
...
//  택배(주소, 종류, 갯수)
	...
	// 과일의 가격을 물어보는 메소드
	void askPrice(String fruit, int count){
		for(int i=0; i<fruitName.length; i++) {
	...
	// 결제하고 거스름돈 내는 메소드
	void payment(String fruit, int count, int price) {
		System.out.println("결제하겠습니다");
		for(int i=0; i<fruitName.length; i++) {
			if(fruitName[i].equals(fruit)) {
				System.out.println("구매 목록: " + fruitName[i] + " " + count + "개");
				System.out.println("총 " + (fruitPrice[i]*count) + "원 입니다");
				System.out.println("내신 돈: " + price + "원, 거스름돈 " + ( price - (fruitPrice[i] * count) )+ "원 입니다");
			}
		}
	}
